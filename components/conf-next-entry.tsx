...
export default function ConfEntry(
  ...
  async function signInWithGitHub() {
    const { data, register, error } = await supabase.auth.signInWithOAuth({
      provider: "github",
    });
  }
  ...
  useEmailQueryParam("login", setUsername);
  ...
  const handleRegister = useCallback(
    async (token?: string) => {
      const res = await register(username, token);
  ...
  return (
    ...
    >
      ...
      <div className="form-widget ml-8">
        <SingleAccordion label="User Details">
